...
1. 训练目标和比赛得分不完全对齐  
   现在更像“尽量别死”，但比赛实际上是“活得久并尽量多拿宝箱”。

2. 观测表达力偏弱  
   21x21 局部地图被压缩过度，宝箱和 buff 信息没有形成有效输入。

3. 动作能力不完整  
   环境支持 16 动作，但当前策略无法使用闪现。
...
在当前仓库版本中，`agent_diy` 已经不再是“纯 PPO 自己摸索”的形态，而是升级为 **PPO + 启发式安全策略混合体**：

1. 模型仍保留 PPO 训练链路，用于持续吸收样本并学习更细粒度的偏好。
2. 动作选择阶段引入了 **安全启发式重排**，优先保证不乱闪、不走死路、危险时先拉开与怪物距离。
3. 在安全窗口内，策略会显式提高对 **宝箱 / buff / 长通道** 的偏好，目标从“只活着”切换为“活满并拿分”。

这种混合式策略适合当前赛题，原因是：

- `gorge_chase` 的动作后果非常即时，局部错误会直接导致暴毙。
- 纯 PPO 前期容易学到“闪现乱用”“短视保命”“不敢拿箱子”的局部最优。
- 先用启发式把底线分数抬起来，再让 PPO 在此基础上持续学习，往往比纯学习方案更快进入 1600~1800 分区间。

### 当前策略原则

1. 危险距离内优先逃生，动作排序由“怪物距离提升幅度 + 通道长度 + 是否死路”主导。
2. 非危险状态下优先追逐 **安全宝箱**，而不是只追最近目标。
3. 闪现必须满足收益条件，常见收益包括：
   - 显著拉开怪物距离
   - 快速到达安全长通道
   - 明显缩短到安全宝箱的距离
4. 训练阶段也使用以贪心为主的混合策略，减少随机探索导致的大量低分样本污染。

### 代码规范补充

1. `agent_ppo` 只作参考基线，不承接业务增强。
2. 所有比赛相关策略增强统一落在 `agent_diy`。
3. `predict` / `exploit` 的任何改动都必须保证：
   - 非法动作概率为 0
   - 概率分布数值稳定
   - 异常时返回合法 fallback 动作，而不是让上层收到 `None`
4. 特征处理除训练特征外，可以额外维护 **policy_context** 一类推理辅助上下文，但必须与环境字段解耦，缺字段时要能降级运行。
5. 奖励项与启发式项的职责分离：
   - 奖励负责长期训练目标
   - 启发式负责短期安全与稳定性
...
当前阶段不再只追求“能跑到 1600”，而是追求：

1. 完成局比例持续提升。
2. 完成局中的平均宝箱数提升到 3 个左右。
3. 最近 100~200 局中能稳定出现 1800+ 分对局，而不是偶发单局。
...
如果想先稳妥落地，建议优先做 **方案 B**，实现更简单，和动作决策更直接相关。
...
2. 闪现很容易被学成“乱交技能”  
   所以必须同时有 `good_flash_reward` 和 `bad_flash_penalty`。
