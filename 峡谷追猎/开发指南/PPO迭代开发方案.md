...
### 7.8 Phase2.8 多尺度地图表达收尾优化

在继续留在 Phase 2 的前提下，当前一个新的明确瓶颈是：

- 环境原始提供的是 `21x21` 视野
- 但 DIY PPO 的模型分支此前只吃 `7x7` 下采样地图
...
- 保留 coarse `7x7`，维持全局通行概览
- 新增 center `11x11`，保留近身细节地形
- 这样既不完全回到超大输入，也不会像单独 `7x7` 那样损失太多局部结构

2. 维度变化

- local map 从 `49` 升级到 `170`
- 总特征维度升级到 `234`

3. 模型同步轻量扩容

- 仍使用 MLP
- 但适当增加隐藏层宽度，避免更大输入被过早压扁
